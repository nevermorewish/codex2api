	...
	priorityLoop:
		for {
			...
			for tierIdx, tier := range fastSchedulerTierOrder {
				...
				if inspectOnly || s.schedulerMode == "occupancy_first" || s.schedulerMode == "remaining_quota" || s.schedulerMode == "fill_first" {
					cursor = &zeroCursor
				}
...
func (s *FastScheduler) scanRangeLocked(expectedTier AccountHealthTier, rangeStart, rangeEnd int, cursor *atomic.Uint64, affinityHash *uint64, apiKeyID int64, exclude map[int64]bool, filter AccountFilter, policy DispatchPolicy, inspectOnly bool, stats *fastSelectionStats) (*Account, bool) {
	...
	// Occupancy-first ranks the whole priority range, while callbacks still run
	// outside the scheduler lock and respect inspect-only admission.
	if s.schedulerMode == "occupancy_first" && affinityHash == nil {
		candidates := make([]fastSchedulerCandidate, 0, rangeLen)
		now := time.Now()
		for offset := 0; offset < rangeLen; offset++ {
			...
			tier, _, limit, _, available := entry.acc.fastSchedulerSnapshotForPolicy(s.baseLimit, now, policy)
			tier, keep := s.normalizeRetainedTier(tier, expectedTier)
			if !keep || tier != expectedTier {
				s.removeLocked(entry.dbID)
				if keep && (s.retainUnavailable || entry.acc.fastSchedulerKeepInPool(s.baseLimit, now, tier, limit, available)) {
			...
			if available && limit > 0 && occupied < limit {
				candidates = append(candidates, fastSchedulerCandidate{acc: entry.acc, occupied: occupied})
			}
		}
		sort.SliceStable(candidates, func(i, j int) bool { return candidates[i].occupied < candidates[j].occupied })
		return s.acquireCandidatesOutsideLock(candidates, expectedTier, apiKeyID, filter, policy, inspectOnly, stats)
	}
