...
func TestOccupancyFirstFilterDoesNotBlockUpdates(t *testing.T) {
	a := newFastSchedulerTestAccount(1, HealthTierHealthy, 100, 4)
	b := newFastSchedulerTestAccount(2, HealthTierHealthy, 100, 4)
	s := NewFastScheduler(4, "occupancy_first")
	s.Rebuild([]*Account{a, b})
	entered, resume := make(chan struct{}), make(chan struct{})
	var once sync.Once
	result := make(chan *Account, 1)
...
func TestOccupancyFirstInspectionDoesNotAcquire(t *testing.T) {
	a := newFastSchedulerTestAccount(1, HealthTierHealthy, 100, 4)
	s := NewFastScheduler(4, "occupancy_first")
	s.Rebuild([]*Account{a})
	for i := 0; i < 10; i++ {
		if got := s.acquireExcludingWithDispatch(0, nil, nil, DispatchPolicyStandard, nil, true); got != a {
			t.Fatal("inspection consumed an account slot")
		}
	}
	if got := accountOccupiedRequests(a); got != 0 {
		t.Fatalf("inspection occupied %d slots", got)
	}
}
