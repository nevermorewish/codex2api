...
func installContinuousRetrySSEKeepaliveWithOptions(c *gin.Context, stream bool, options continuousRetrySSEKeepaliveOptions) func() {
	...
	informational, unwrapped := unwrapHTTPResponseWriter(responseWriter)
	keepalive := &requestContinuousRetryKeepalive{ctx: requestCtx, write: func() error {
		// Preserve the fork's pre-content HTTP error semantics. Once output
		// commits, use upstream protocol-specific SSE comments or Messages pings.
		if unwrapped && informational != nil && !c.Writer.Written() {
