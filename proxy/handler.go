...
func (h *Handler) logUsageForRequest(c *gin.Context, input *database.UsageLogInput) {
	if c != nil && input != nil {
		if input.AccountID > 0 && input.StatusCode >= 400 && canFallbackAfterPrimaryDeadline(c) {
...
func (h *Handler) Responses(c *gin.Context) {
	...
	nextAttempt := 0
	runAttempts := func() {
		for attempt := nextAttempt; ; attempt++ {
			...
			if !retainedHTTPFallback {
				...
				if fallbackState.usingFallback() {
					...
					account, stickyProxyURL, affinityGuard, selectionErr = h.nextFallbackAwareAccountWithGuard(c.Request.Context(), fallbackState, affinityKey, apiKeyID, retryExclusions, accountFilter, dispatchPolicy)
				} else {
			...
			if account.IsRelayStyle() {
				...
				if resp.StatusCode != http.StatusOK {
					...
					if !account.IsExternalFallback() && !invalidEncryptedContentRetried && isInvalidEncryptedContentError(resp.StatusCode, errBody) && len(grokCompactionDigestsForAccount(upstreamCtx, account)) == 0 {
	...
	runAttempts()
	if handoffPrimaryDeadlineToFallback(c, fallbackState) {
		runAttempts()
	}
}
...
func (h *Handler) ResponsesCompact(c *gin.Context) {
	...
	accountFilter = h.withModelCooldownFilter(selectionCtx, effectiveModel, accountFilter)
	accountFilter = excludeClaudeAccountsFilter(accountFilter)
	if continuationUnavailable {
		accountFilter = relayOnlyAccountFilter(accountFilter)
	}
	...
	runAttempts := func() {
		for attempt := nextAttempt; ; attempt++ {
			...
			if isGrokNativeRouteResponse(resp) || nativePassthroughProtocol(resp) != "" {
				...
				streamAttempt := h.newContinuousRetryStreamAttempt(isStream && continuousRetryBuffersAttempts(continuousRetryPolicy), c.Writer, downstreamFlusher)
				usage, outcome, wroteAnyBody, firstTokenMs := forwardGrokNativeResponseTo(readCtx, c, resp, GrokProtocolChatCompletions, isStream, start, ttftGuard.Stop, streamAttempt.writerOr(output), streamAttempt.flusherOr(downstreamFlusher))
				totalDuration := int(time.Since(start).Milliseconds())
				ttftGuard.Stop()
				resp.Body.Close()
