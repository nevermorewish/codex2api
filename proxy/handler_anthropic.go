		...
		for attempt := nextAttempt; ; attempt++ {
			...
			if !retainedHTTPFallback {
				...
				if fallbackState.usingFallback() {
					...
					account, stickyProxyURL, affinityGuard, selectionErr = h.nextFallbackAwareAccountWithGuard(c.Request.Context(), fallbackState, affinityKey, apiKeyID, retryExclusions, accountFilter, auth.DispatchPolicyStandard)
				} else {
