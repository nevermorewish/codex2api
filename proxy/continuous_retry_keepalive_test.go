...
// Informational keepalives leave the final HTTP status selectable until content.
func TestContinuousRetrySSEKeepaliveBeforeFirstEventThenContent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/stream", func(c *gin.Context) {
		...
		if c.Writer.Written() {
			t.Error("pre-content heartbeat committed final status")
			return
		}
		setSSEStreamHeaders(c, "text/event-stream")
	...
	if resp.StatusCode != http.StatusOK || string(body) != "data: content\n\n"+continuousRetryKeepaliveComment {
