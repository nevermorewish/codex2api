...
export default function RobotMonitor() {
  ...
  useEffect(() => {
    api.getSettings().then((next) => setSettings({ ...next, feishu_alert_error_codes: next.feishu_alert_error_codes?.trim() || '400-599' })).catch((e) => setError(e instanceof Error ? e.message : '加载失败'))
  }, [])

  const patch = async (data: Partial<SystemSettings>) => {
    setSaving(true)
    try {
  ...
  if (!settings) {
    return <StateShell variant="page" loading error={error}><></></StateShell>
  }

  const saveAll = () => void patch({
